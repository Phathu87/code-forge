...
- Deployed commit: e08ca06d75bf0b222924bf3da01b0caf3c09a922
- Render service: srv-dalheiu5vjqs73fdqg90, free plan, Frankfurt
- Render deployment: dep-dalhm8bm8hqs739kfr10; live at 2026-09-16 22:42:35 UTC
...
The deployed root returned HTTP 200 with the CodeForge title. /api/health returned ok, version 0.1.0 and the commit above after a successful database query. /api/config returned registrationEnabled false. GitHub Checks passed for this revision: https://github.com/Phathu87/code-forge/actions/runs/35158691804.
...
Brevo HTTPS integration is implemented, but the API key and verified sender are saved in Render with owner approval. Authorizing the service outbound IP ranges and enabling registration for the owner and a test alias await approval. Actual verification/recovery delivery checks are pending. Keep registration closed until mail works, then restrict access to named test addresses with REGISTRATION_ALLOWLIST. Public registration is intentionally rejected by the server without this allowlist.
